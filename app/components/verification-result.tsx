...
export function VerificationResult({ status, dimensions, limitations }: { status: VerificationStatus; dimensions: DimensionResult[]; limitations: string[] }) {
  return <section className="verification-result" aria-live="polite">
    <header className="result-header"><div><p className="eyebrow">Package result</p><h2>{status}</h2></div><StatusPill status={status}/></header>
